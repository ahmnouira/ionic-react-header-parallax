...
import { UseIonHeaderParallaxInput, UseIonHeaderParallaxResult } from "./index";
...
export function useIonHeaderParallax({
  ...
  const initElements = () => {
    ...
    if (!parentElement) throw new Error(ERRORS["ion-page-required"]);

    // ion-toolbar
    const toolbar = header.querySelector("ion-toolbar") as HTMLElement;
    if (!toolbar) throw new Error(ERRORS["ion-toolbar-required"]);
    ...
    if (!ionContent) throw new Error(ERRORS["ion-content-required"]);
    ...
    if (!originalToolbarBgColor) {
      throw new Error(ERRORS["toolbar-background-required"]);
    }
